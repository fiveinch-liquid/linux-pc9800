	     ...
	     unsigned int cmd, unsigned long arg)
{
	...
	case KDKBDREP:
		if (arg) {
			...
			if (copy_from_user (&kbdrep, (struct kbd_repeat *) arg,
					    sizeof (struct kbd_repeat)))
				return -EFAULT;
			printk (KERN_DEBUG
				"keyboard: ioctl (KDKBDREP, { %d, %d })\n",
				kbdrep.delay, kbdrep.rate);
			if (kbdrep.delay < 0 && kbdrep.rate < 0)
				return 0;	/* implementation check */
			if (!perm)
				return -EPERM;
			if ((e = setrepeat (&kbdrep)))
				return e;
			if (copy_to_user ((struct kbd_repeat *) arg, &kbdrep,
					  sizeof (struct kbd_repeat)))
				return -EFAULT;
		}
		return 0;	/* also implementation check */
#endif
